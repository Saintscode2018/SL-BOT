...
- Successful mutations, offer acknowledgements, all read-only/reporting responses (`/health`, `/setup view`, `/team list`, `/limit view`, `/staff list`, `/roster`, `/teamhealth`, and `/folist`), and all handled errors are ephemeral and private to the invoking user.
