...
`/team disband` is a `STAFF_ONLY` team mutation available only to database `BOTPERM`/`BOTPERM_ADMIN` holders. Ordinary TM/ATM/PM appointments, the guild owner, Discord Administrators, the legacy role, players, and unrelated members have no access.

Global access: guild-scoped database `BOTPERM` or `BOTPERM_ADMIN` only. Guild ownership, Discord Administrator permission, and the legacy configured `bot_permissions` role do not authorize after bootstrap. Club staff scope remains an active database TM/ATM/PM appointment. Explicit channel scopes are subcommand-aware: `/health`, `/team list`, `/staff list`, `/roster view`, `/limit view`, `/offer`, `/demand`, `/release`, `/promote`, `/demote`, and the two read-only permission views are `BOT_OR_STAFF`; setup/team/staff/limit mutations, `/roster add`, `/roster remove`, setup view, `/teamhealth`, `/folist`, and debug reset are `STAFF_ONLY`. Roster add/remove accept global access only, never club staff scope. Command-channel access never grants command authority. Transfer Market and Audit are output-only bot-operation channels. Non-global callers receive channel guidance mentioning only Bot Commands, while globally authorized callers receive concise guidance mentioning both allowed channels. Channel-not-configured errors remain distinct.
...
The shared identity is used by team, staff, roster, limit, offer, and conflict output. Normal output remains `<emoji> <@&roleId>`. Read-only/reporting responses from `/health`, `/setup view`, `/team list`, `/limit view`, `/staff list`, `/roster view`, `/teamhealth`, and `/folist` are ephemeral and private to the invoking user, including chunked follow-ups. Staff directories are vertical and chunked safely. `/roster view team:<team>` replaces standalone `/roster team:<team>`. Rosters have no title; their description begins `<emoji> <@&roleId> Roster`, contains no separate `Team` field, and ends with `Roster for <footer-safe identity>, <server name>`; custom footer emoji use `.emojiName.`. Thumbnails derive only from emoji.

`RosterAdministrationService` implements `/roster add player:<user> team:<team>` and `/roster remove player:<user>`. Add validates a non-bot free agent, rejects active staff, requires an active guild-local team, and enforces the team override/guild default/fallback squad limit. Remove derives exactly one active player team and rejects free agents, staff, and ambiguous membership corruption. A player has an active `PLAYER` row and the team-specific Discord role, never a global Player role. Only that team role is changed; unrelated and staff roles remain. Discord changes occur before the repeated database transaction, with compensation on commit failure and surfaced/logged compensation failure. Signing/release transaction and audit-domain history are written, but Audit + Transfer channel routing is deferred to the next hotfix.
...
`TeamHealthService` supplies a read-only health model with no handler-level Prisma access. It counts active `PLAYER` rows exactly like roster/capacity logic and derives effective limits from the team override or guild default. Compact mode reuses active-team creation/ID order and chunks full `<emoji> <role>: count 👤, heart` rows; detailed mode adds active staff slots in one continuous blockquote. The pure heart mapping is 🖤 0–4, 💛 5–9, 💚 10–15, and ❤️ 16+. The command is ephemeral, global-admin-only, and Staff-only. Role and unique staff-user metadata use cache-then-fetch resolution, selected stale roles are rejected, and detailed embeds use the live role color and emoji thumbnail.

`FranchiseOwnerListService` supplies a read-only list of active teams and their active Team Manager appointments in creation/ID order. Inactive teams and non-TM memberships are excluded. `/folist` formats each line as `<emoji> <@&roleId> Team Manager: <formatted manager or Vacant>`, using `formatUserWithVisibleName` (`<@USER_ID> \`VisibleName\``) for assigned managers. The command is ephemeral, Staff-only, and global-admin-only, chunked across embeds with exact title `Franchise Owner List`. Missing team roles throw mapped errors, and unconfigured servers show an empty-state embed (`No active teams are currently configured.`).
...
Stage 4C.2 extends that coordinator with ordered multi-member execution for `/team disband`. Active memberships are grouped by Discord user and role IDs are deduplicated. Every affected member loses the team-specific role; TM/ATM/PM members also lose their matching configured global staff role, while ordinary players lose no unrelated role. A failure on any member compensates all earlier members before surfacing, and database failure compensates the complete applied batch in reverse order.

`TeamDisbandmentService` repeats guild/team/activity and exact membership-set validation inside one Prisma transaction, ends all active rows, expires only related `PENDING` offers, marks the team inactive, and writes `team.disbanded` audit metadata. It never deletes the team, role identity, emoji, users, memberships, transactions, offers, or audits. The command handler owns the ephemeral warning/success cards and initiating-user/guild-bound two-minute confirmation; authorization and Staff-channel policy are checked again when confirming. `/team remove` is no longer registered.
