...
4. With `SLBOT_ENABLE_DEBUG_COMMANDS=true`, verify `/debugreset` requires a database `BOTPERM`/`BOTPERM_ADMIN` at both prompt and confirmation time, preserves permission records to prevent lockout, and retains its confirmation safety checks.
...
10. In Staff Commands as database `BOTPERM` and `BOTPERM_ADMIN` holders, run `/teamhealth` and `/teamhealth team:<team>`. Confirm every initial and chunked continuation response is private to the invoking user. Verify compact active-only order/count/heart rows and detailed TM/ATM/PM vacancies, effective limit, role color/mention, emoji/thumbnail, guild author, and actor UTC footer. Restart/clear caches and confirm role/user names fetch correctly. Verify owner/Admin/legacy-role/ordinary TM/ATM/PM/player denial, wrong-channel denial, inactive/foreign/unknown/stale-role errors, 16 and above remaining ❤️, and enough teams to produce multiple untruncated embeds.
11. In Staff Commands as database `BOTPERM` and `BOTPERM_ADMIN` holders, run `/folist`. Confirm every initial and chunked continuation response is private to the invoking user. Verify exact compact row format (`<emoji> <role mention> Team Manager: <formatted manager or Vacant>`), active-only creation/ID order, cold-cache role/user resolution, title `Franchise Owner List`, empty state (`No active teams are currently configured.`), multi-embed chunking without line splits, and denial of owner/Admin/legacy-role/ordinary staff/players and wrong channels.
