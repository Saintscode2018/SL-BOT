...
8. Verify `/roster` is visible only to the invoking user, has no title, begins `<emoji> <@&roleId> Roster`, contains no `Team` field, and retains the effective count, exact TM/ATM/PM headings, players, emoji thumbnail, author, and `Roster for <team>, <server>` footer.
9. Verify team limit update/reset output remains unchanged; confirm `/limit view` is visible only to the invoking user, uses only identity, and keeps the current nonzero Discord role color with the existing zero/missing-color fallback.
