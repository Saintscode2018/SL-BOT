...
19. In Staff Commands as the server owner, Administrator, and Bot Permissions member, run `/folist`. Verify exact compact row format (`<emoji> <role mention> Team Manager: <formatted manager or Vacant>`), active-only creation/ID order, cold-cache role/user resolution, title `Franchise Owner List`, empty state (`No active teams are currently configured.`), multi-embed chunking without line splits, and denial of ordinary staff/players and wrong channels.
