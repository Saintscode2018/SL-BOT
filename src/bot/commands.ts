  ...
  async execute(interaction, context) {
    ...
    if (subcommand === 'appoint') {
      ...
      const embed = createSuccessEmbed({
        ...
        description: `Successfully appointed <@${result.user.discordUserId}> as **${positionName}**.`,
    ...
    if (subcommand === 'remove') {
      ...
      const embed = createSuccessEmbed({
        ...
        description: `Successfully removed active **${positionName}** position.`,
    ...
    if (selectedTeamId) {
      ...
      const fields = (['TEAM_MANAGER', 'ASSISTANT_MANAGER', 'PLAYER_MANAGER'] as const).map(
        (type) => ({
          name: getFriendlyPositionName(type),
    ...
    const fields = await Promise.all(
      activeTeams.map(async ({ club }) => {
        ...
        const staffText = (['TEAM_MANAGER', 'ASSISTANT_MANAGER', 'PLAYER_MANAGER'] as const)
          .map(
            (t) =>
              `**${getFriendlyPositionName(t)}**: ${byType.has(t) ? `<@${byType.get(t)?.discordUserId}>` : 'Vacant'}`,
          )
          .join(' | ');
